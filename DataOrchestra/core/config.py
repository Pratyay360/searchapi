...
class OutputFormat(str, Enum):
    """Output formats with validation."""
    TEXT = "text"
    JSON = "json"
    MARKDOWN = "markdown"
    YAML = "yaml"
...
class CacheConfig(BaseModel):
    ...
    memory_max_size: int = Field(default=1000, ge=1, le=10000, description="Maximum memory cache size")
    disk_cache_dir: Optional[DirectoryPath] = Field(default=None, description="Disk cache directory")
...
class WebConfig(BaseModel):
    """Web crawling configuration with validation."""
    timeout: float = Field(default=12.0, ge=1.0, le=300.0, description="Request timeout in seconds")
    max_retries: int = Field(default=4, ge=0, le=10, description="Maximum retry attempts")
    backoff_factor: float = Field(default=0.6, ge=0.1, le=2.0, description="Backoff multiplier")
    delay_range: tuple[float, float] = Field(default=(1.0, 3.0), description="Delay range in seconds")
    workers: int = Field(default=5, ge=1, le=20, description="Number of concurrent workers")
    max_pages: int = Field(default=4000, ge=1, le=10000, description="Maximum pages to crawl")
...
class ProcessingConfig(BaseModel):
    """Text processing configuration with validation."""
    min_word_length: int = Field(default=2, ge=1, le=10, description="Minimum word length to keep")
    min_sentence_length: int = Field(default=10, ge=5, le=100, description="Minimum sentence length to keep")
    max_token_limit: int = Field(default=5000, ge=100, le=50000, description="Maximum token limit for splitting")
    noise_words: set[str] = Field(
        default_factory=lambda: {"http", "https", "www", "com", "org", "contact", "follow"},
    ...
    language_detection_enabled: bool = Field(default=True, description="Enable automatic language detection")
    quality_threshold: float = Field(default=0.5, ge=0.0, le=1.0, description="Minimum quality threshold")
    preserve_case: bool = Field(default=False, description="Preserve original case")
    remove_duplicates: bool = Field(default=True, description="Remove duplicate sentences/phrases")
...
class DownloadConfig(BaseModel):
    ...
    timeout: float = Field(default=15.0, ge=1.0, le=300.0, description="Download timeout in seconds")
    chunk_size: int = Field(default=8192, ge=1024, le=1048576, description="Download chunk size in bytes")
    max_workers: int = Field(default=3, ge=1, le=10, description="Maximum concurrent downloads")
    retry_attempts: int = Field(default=3, ge=1, le=10, description="Retry attempts for downloads")
...
class SecurityConfig(BaseModel):
    ...
    enable_path_validation: bool = Field(default=True, description="Enable path traversal protection")
    enable_url_validation: bool = Field(default=True, description="Enable URL validation")
    allowed_domains: Optional[set[str]] = Field(default=None, description="Allowed domains for web scraping")
    blocked_domains: Optional[set[str]] = Field(default=None, description="Blocked domains for web scraping")
    max_url_length: int = Field(default=2048, ge=100, le=8192, description="Maximum URL length")
    enable_audit_logging: bool = Field(default=True, description="Enable security audit logging")
    sanitize_user_input: bool = Field(default=True, description="Sanitize all user input")
...
class LoggingConfig(BaseModel):
    ...
    file_output: Optional[DirectoryPath] = Field(default=None, description="Log file output directory")
    console_output: bool = Field(default=True, description="Enable console output")
    max_file_size: Optional[int] = Field(default=None, ge=1, description="Maximum log file size in bytes")
...
class MonitoringConfig(BaseModel):
    ...
    metrics_retention_days: int = Field(default=30, ge=1, le=365, description="Days to retain metrics")
    enable_performance_profiling: bool = Field(default=False, description="Enable performance profiling")
    health_check_interval: int = Field(default=300, ge=60, le=3600, description="Health check interval in seconds")
...
class Config(BaseModel):
    ...
    def validate_config_consistency(cls, values):
        ...
        if logging_config.get('file_output') and not logging_config.get('file_output').parent.exists():
    ...
    def from_env(cls) -> 'Config':
        ...
        if min_word_length := os.getenv("DATAORCHESTRA_MIN_WORD_LENGTH"):
            config_dict.setdefault('processing', {})['min_word_length'] = int(min_word_length)
        
        if max_token_limit := os.getenv("DATAORCHESTRA_MAX_TOKEN_LIMIT"):
            config_dict.setdefault('processing', {})['max_token_limit'] = int(max_token_limit)
    ...
    def get_effective_config(self) -> Dict[str, Any]:
        """Get effective configuration after applying all defaults and overrides."""
        return self.to_dict()
    ...
    def get_value(self, path: str, default: Any = None) -> Any:
        """Get a configuration value by dot path (e.g., 'web.timeout')."""
        keys = path.split('.')
        value = self
    ...
    def set_value(self, path: str, value: Any) -> None:
        """Set a configuration value by dot path (e.g., 'web.timeout')."""
        keys = path.split('.')
        config_obj = self
...
def load_config_from_file(config_path: Path) -> Config:
    ...
    config = Config.from_yaml(config_path) if config_path.suffix.lower() in ['.yml', '.yaml'] else Config.from_json(config_path)
    set_config(config)
    return config
