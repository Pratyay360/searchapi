...
class TextCleaner(FileProcessor):
    ...
    def process_file(self, file_path: Path) -> FileProcessingResult:
        try:
            ...
            if text is None:
                print(f"Cannot decode {file_path.name} with any supported encoding")
            ...
            return FileProcessingResult(
                success=True, input_path=file_path, metadata=metadata
            )
...
class DocumentConverter(FileProcessor):
    def __init__(self):
        super().__init__(
            name="DocumentConverter", supported_extensions={".docx", ".odt", ".doc"}
        )
    ...
    def convert_to_text(self, source_path: Path, target_path: Path) -> None:
        if source_path.suffix.lower() == ".docx":
            ...
            print(f"Unsupported document format: {source_path.suffix}")
    ...
    def _convert_docx(self, source_path: Path, target_path: Path) -> None:
        try:
            ...
            print(f"Failed to convert DOCX {source_path.name}: {str(e)}")
    ...
    def process_directory(
        ...
        with LogContext(
            ...
            supported_extensions = (
                self.cleaner.supported_extensions | self.converter.supported_extensions
            )
                    ...
                    try:
                        ...
                        results.append(
                            FileProcessingResult(
                                success=False, error=str(e), input_path=file_path
                            )
                        )
