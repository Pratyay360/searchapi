...
class TrackerPatternsDB:
    ...
    def add(self, rule: RuleType) -> None:
        key = rule[self.Fields.url_regexp]
        value = (
            rule[self.Fields.url_ignore],
            rule[self.Fields.del_args],
    ...
    def iter_clear_list(self) -> Iterator[RuleType]:
        ...
        for url in self.CLEAR_LIST_URL:
            ...
            try:
                ...
                log.warning(
                    "TRACKER_PATTERNS: HTTPError (%s) occured while fetching %s",
                    url,
                    exc,
                )
                continue
            ...
            if resp.status_code != 200:
                log.warning(
                    f"TRACKER_PATTERNS: ClearURL ignore HTTP {resp.status_code} {url}"
                )
                continue
            ...
            if query_args:
                ...
                for name, val in query_args.copy():
                    ...
                    for pattern in rule[self.Fields.del_args]:
                        if re.match(pattern, name):
                            log.debug(
                                "TRACKER_PATTERNS: %s remove tracker arg: %s='%s'",
                ...
                for pattern in rule[self.Fields.del_args]:
                    if re.match(pattern, query_str):
                        log.debug(
                            "TRACKER_PATTERNS: %s remove tracker arg: '%s'",
                            parsed_new_url.netloc,
                            query_str,
                        )
