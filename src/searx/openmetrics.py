...
class OpenMetricsFamily:  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        self,
        key: str,
        type_hint: OMFTypeHintType,
        help_hint: str,
        data_info: OMFDataInfoType,
        data: list[t.Any],
    ) -> None:
        self.key: str = key
...
"""
        ...
        for i, data_info_dict in enumerate(self.data_info):
            ...
            info_representation = ",".join(
                [f'{key}="{value}"' for (key, value) in data_info_dict.items()]
            )
            text_representation += (
                f"{self.key}{{{info_representation}}} {self.data[i]}\n"
            )
