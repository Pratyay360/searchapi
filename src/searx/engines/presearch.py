...
def _get_request_id(query, params) -> tuple[str, Cookies]:
    ...
    if params["searxng_locale"] != "all":
        ...
        if l and l.territory:
            headers["Accept-Language"] = (
                f"{l.language}-{l.territory},{l.language};q=0.9,*;q=0.5"
            )
...
def parse_search_query(json_results):
    ...
    for item in (
        json_results.get("specialSections", {})
        .get("topStoriesCompact", {})
        .get("data", [])
    ):
        result = {
            "url": item["link"],
