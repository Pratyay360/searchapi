...
CAPTCHA_PATTERN = (
    r'\{[^{]*?"action"\s*:\s*"captcha"\s*,\s*"url"\s*:\s*"([^"]+)"[^{]*?\}'
)


def is_alibaba_captcha(html) -> bool:
    """
...
def response(resp):
    ...
    if is_alibaba_captcha(text):
        raise SearxEngineCaptchaException(
            suspended_time=900,
            message="Alibaba CAPTCHA detected. Please try again later.",
        )

    if quark_category == "images":
        data = json.loads(text)
        for item in (
            data.get("data", {}).get("hit", {}).get("imgInfo", {}).get("item", [])
        ):
            try:
...
def parse_ss_doc(data):
    ...
    try:
        thumbnail = (
            data.get("picListProps", [])[0].get("src").replace("http://", "https://")
        )
