...
def search(query, params) -> EngineResults:
    ...
    reader_thread = Thread(
        target=_get_results_from_process, args=(res, cmd, params["pageno"])
    )
    reader_thread.start()
...
def _get_command_to_run(query) -> list[str]:
...
def _get_results_from_process(res: EngineResults, cmd, pageno) -> EngineResults | None:
    leftover = ""
    count = 0
    ...
    with Popen(cmd, stdout=PIPE, stderr=PIPE, env=environment_variables) as process:
        ...
        while line:
            ...
            for raw_result in raw_results:
                ...
                if start <= count <= end:  # pylint: disable=chained-comparison
...
def __check_query_params(params: list[str]) -> None:
    if not query_type:
        return
    ...
    if query_type == "path":
        ...
        for param in params:
            if param not in query_enum:
                raise ValueError(
                    "submitted query params is not allowed",
                    param,
                    "allowed params:",
                    query_enum,
                )
...
def check_parsing_options(engine_settings) -> None:
    ...
    if "delimiter" in engine_settings and (
        "chars" not in engine_settings["delimiter"]
        or "keys" not in engine_settings["delimiter"]
    ):
        raise ValueError
...
def __parse_single_result(raw_result):
    ...
    if delimiter:
        elements = raw_result.split(
            delimiter["chars"], maxsplit=len(delimiter["keys"]) - 1
        )
