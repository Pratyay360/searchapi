...
def request(query, params):
    ...
    params["cookies"]["preferences"] = "N1N".join(
        ["{}EEE{}".format(*x) for x in cookie.items()]
    )
...
def response(resp):
    ...
    results_raw = (
        "{"
        + extr(resp.text, f"React.createElement(UIStartpage.AppSerp{categ}, {{", "}})")
        + "}}"
    )
...
def fetch_traits(engine_traits: EngineTraits) -> None:
    ...
    for eng_tag in sp_region_names:
        ...
        if "-" in babel_region_tag:  # pyright: ignore[reportOperatorIssue]
            ...
            try:
                ...
                print(
                    f"IGNORE: can't determine babel locale of startpage's locale {eng_tag}"
                )
                continue
    ...
    for lang_code in filter(
        ...
        if not native_name:
            print(
                f"IGNORE: language name of startpage's language {lang_code} is unknown by babel"
            )
            continue
