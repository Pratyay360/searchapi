...
from searx.result_types.answer import WeatherAnswer
...
def _weather_data(
    location: weather.GeoLocation, data: dict[str, t.Any]
) -> WeatherAnswer.Item:
...
def response(resp) -> EngineResults:
    ...
    for day in json_data["weather"]:
        ...
        for index, forecast in enumerate(day["hourly"]):
            ...
            forecast_data.datetime = weather.DateTime(
                date.replace(hour=index * time_slot_len + 1)
            )
