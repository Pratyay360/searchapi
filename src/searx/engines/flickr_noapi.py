...
modelexport_re = re.compile(r"^\s*modelExport:\s*({.*}),$", re.MULTILINE)


def build_flickr_url(user_id, photo_id) -> str:
    return photo_url.format(userid=user_id, photoid=photo_id)


def _get_time_range_url(time_range) -> str:
...
def response(resp):  # pylint: disable=too-many-branches
    ...
    for x, index in enumerate(legend):
        ...
        photo = model_export["main"][index[0]][int(index[1])][index[2]][index[3]][
            index[4]
        ][index[5]][int(index[6])][index[7]]
        ...
        if not size_data:
            logger.debug(
                "cannot find valid image size: {}".format(repr(photo["sizes"]["data"]))
            )
            continue
