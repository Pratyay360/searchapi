...
from searx.utils import eval_xpath, eval_xpath_list, extr, extract_text, humanize_bytes
...
title_xpath = (
    './/h3[@class="b-serp-item__title"]/a[@class="b-serp-item__title-link"]/span'
)
content_xpath = './/div[@class="b-serp-item__content"]//div[@class="b-serp-item__text"]'


def catch_bad_response(resp) -> None:
...
def response(resp):
    ...
    if search_type == "images":
        ...
        if content_between_tags == "fail":
            content_between_tags = extr(
                html_sample, '{"location":"/images/search/', "false}}}"
            )
            json_data = (
                '{"location":"/images/search/' + content_between_tags + "false}}}"
            )

        json_resp = loads(json_data)

        results = []
        for item_data in json_resp["initialState"]["serpList"]["items"][
            "entities"
        ].values():
            ...
            for i in (
                item_data["viewerData"]["dups"] + item_data["viewerData"]["preview"]
            ):
