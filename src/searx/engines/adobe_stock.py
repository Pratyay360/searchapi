    ...
    if isinstance(adobe_content_types, list):
        for t in adobe_content_types:
            if t not in ADOBE_VALID_TYPES:
                raise ValueError(
                    f"adobe_stock engine: adobe_content_types: '{t}' is invalid"
                )
    else:
        raise ValueError(
            f"adobe_stock engine: adobe_content_types must be a list of strings not {type(adobe_content_types)}"
        )
...
def request(query, params):
    ...
    for content_type in ADOBE_VALID_TYPES:
        args[f"filters[content_type:{content_type}]"] = (
            1 if content_type in adobe_content_types else 0
        )
...
def parse_audio_item(item):
    ...
    return {
        ...
        "publishedDate": (
            datetime.fromisoformat(audio_data["release_date"])
            if audio_data["release_date"]
            else None
        ),
        "length": (
            timedelta(seconds=round(audio_data["duration"] / 1000))
