...
def response(resp: "SXNG_Response") -> list[dict[str, t.Any]]:
    ...
    for item in eval_xpath_list(
        dom, '//ol[@id="b_results"]/li[contains(@class, "b_algo")]'
    ):
...
def fetch_traits(engine_traits: EngineTraits) -> None:
    ...
    for href in eval_xpath(
        dom, '//div[@id="region-section-content"]//div[@class="regionItem"]/a/@href'
    ):
        ...
        for lang_tag in babel.languages.get_official_languages(cc_tag, de_facto=True):
            ...
            try:
                sxng_tag = region_tag(
                    babel.Locale.parse(f"{lang_tag}_{cc_tag.upper()}")
                )
