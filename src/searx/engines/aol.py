...
def _general_results(doc: html.HtmlElement) -> EngineResults:
    ...
    for result in eval_xpath_list(
        doc, "//div[@id='web']//ol/li[not(contains(@class, 'first'))]"
    ):
        ...
        res.add(
            res.types.MainResult(
                ...
                content=extract_text(
                    eval_xpath(result, ".//div[contains(@class, 'compText')]")
                )
                or "",
                thumbnail=extract_text(
                    eval_xpath(result, ".//a[contains(@class, 'thm')]/img/@data-src")
                )
                or "",
            )
        )
...
def _video_results(doc: html.HtmlElement) -> EngineResults:
    ...
    for result in eval_xpath_list(doc, "//div[contains(@class, 'results')]//ol/li"):
        ...
        published_date_raw = extract_text(
            eval_xpath(result, ".//div[contains(@class, 'v-age')]")
        )
        try:
            ...
            res.types.LegacyResult(
                {
                    ...
                    "thumbnail": extract_text(
                        eval_xpath(result, ".//img[contains(@class, 'thm')]/@src")
                    ),
                    "length": extract_text(
                        eval_xpath(result, ".//span[contains(@class, 'v-time')]")
                    ),
...
def response(resp: "SXNG_Response") -> EngineResults:
    ...
    for suggestion in eval_xpath_list(
        doc, ".//ol[contains(@class, 'searchRightBottom')]//table//a"
    ):
        results.add(
            results.types.LegacyResult({"suggestion": extract_text(suggestion)})
        )
