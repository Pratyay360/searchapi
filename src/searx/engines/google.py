...
def detect_google_sorry(resp: "SXNG_Response") -> None:
    ...
    for result in eval_xpath_list(dom, "//a[@data-ved and not(@class)]"):
        ...
        try:
            ...
            content_nodes = eval_xpath(
                result, '../..//div[contains(@class, "ilUpNd H66NU aSRlid")]'
            )
...
def fetch_traits(engine_traits: EngineTraits, add_domains: bool = True) -> None:
    ...
    for x in eval_xpath_list(dom, "//select[@name='hl']/option"):
        ...
        try:
            ...
            print(
                f"INFO:  google UI language {eng_lang} ({x.text.split('(')[0].strip()}) is unknown by babel"
            )
            continue
