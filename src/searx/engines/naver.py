...
def request(query, params):
    ...
    if naver_category in naver_category_dict:
        query_params["start"] = (params["pageno"] - 1) * naver_category_dict[
            naver_category
        ]["start"] + 1
...
def parse_general(data) -> EngineResults:
    results = EngineResults()

    dom = html.fromstring(data)

    for item in eval_xpath_list(
        dom, "//ul[contains(@class, 'lst_total')]/li[contains(@class, 'bx')]"
    ):
        thumbnail = None
        try:
            thumbnail = eval_xpath_getindex(
                item, ".//div[contains(@class, 'thumb_single')]//img/@data-lazysrc", 0
            )
        ...
        results.add(
            MainResult(
                title=extract_text(
                    eval_xpath(item, ".//a[contains(@class, 'link_tit')]")
                ),
                url=eval_xpath_getindex(
                    item, ".//a[contains(@class, 'link_tit')]/@href", 0
                ),
...
def parse_images(data) -> list[dict[str, str]]:
    results = []
...
def parse_news(data) -> EngineResults:
    results = EngineResults()
    dom = html.fromstring(data)
...
def parse_videos(data):
    ...
    for item in eval_xpath_list(dom, "//li[contains(@class, 'video_item')]"):
        ...
        try:
            thumbnail = eval_xpath_getindex(
                item, ".//img[contains(@class, 'thumb')]/@src", 0
            )
        except (ValueError, TypeError, SearxEngineXPathException):
            pass
        ...
        try:
            length = parse_duration_string(
                extract_text(eval_xpath(item, ".//span[contains(@class, 'time')]"))
            )
        ...
        results.append(
            {
                ...
                "title": extract_text(
                    eval_xpath(item, ".//a[contains(@class, 'info_title')]")
                ),
                "url": url,
                "thumbnail": thumbnail,
                "length": length,
