...
def response(resp) -> list[dict[str, str]]:
    results = []
    first_result = True
    ...
    for page in search_results.get("data", []):
        ...
        parts_of_speech = page.get("senses") and page["senses"][0].get(
            "parts_of_speech"
        )
...
def get_definitions(page) -> list[tuple[str, str, str]]:
    # Process definitions
    definitions = []
    for defn_raw in page["senses"]:
        ...
        if defn_raw.get("restrictions"):
            extra.append(
                "Only applies to: " + ", ".join(defn_raw["restrictions"]) + ". "
            )
        definitions.append(
            (
...
def get_infobox(alt_forms, result_url: str, definitions: list[tuple[str, str, str]]):
    infobox_content = []
    # title & alt_forms
