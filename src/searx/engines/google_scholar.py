...
def request(query: str, params: "OnlineParams") -> None:
    ...
    params["url"] = (
        "https://" + google_info["subdomain"] + "/scholar?" + urlencode(args)
    )
...
def response(resp: "SXNG_Response") -> EngineResults:  # pylint: disable=too-many-locals
    ...
    if resp.status_code in (301, 302, 303, 307, 308) and "Location" in resp.headers:
        ...
        raise httpx.TooManyRedirects(
            f"location {resp.headers['Location'].split('?')[0]}"
        )
    ...
    for result in eval_xpath_list(dom, "//div[@data-rp]"):
        ...
        pub_type: str = (
            extract_text(eval_xpath(result, ".//span[@class='gs_ctg2']")) or ""
        )
        if pub_type:
        ...
        html_url: str = ""
        pdf_url: str = ""
        doc_url = eval_xpath_getindex(
            result, ".//div[@class='gs_or_ggsm']/a/@href", 0, default=None
        )
...
def detect_google_captcha(dom: ElementType) -> None:
