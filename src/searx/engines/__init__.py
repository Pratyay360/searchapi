...
def check_engine_module(module: types.ModuleType) -> None:
...
def set_loggers(engine: "Engine|types.ModuleType", engine_name: str) -> None:
...
def update_engine_attributes(
    engine: "Engine | types.ModuleType", engine_data: dict[str, t.Any]
) -> None:
...
def update_attributes_for_tor(engine: "Engine | types.ModuleType") -> None:
...
def is_missing_required_attributes(engine: "Engine | types.ModuleType") -> bool:
    ...
    for engine_attr in dir(engine):
        if not engine_attr.startswith("_") and getattr(engine, engine_attr) is None:
            logger.error(
                'Missing engine config attribute: "{0}.{1}"'.format(
...
def using_tor_proxy(engine: "Engine | types.ModuleType"):
    ...
    return settings["outgoing"].get("using_tor_proxy") or getattr(
        engine, "using_tor_proxy", False
    )


def is_engine_active(engine: "Engine | types.ModuleType") -> bool:
...
def call_engine_setup(
    ...
    if not setup_ok:
        logger.error(
            "%s: Engine setup was not successful, engine is set to inactive.",
            engine.name,
        )
    return setup_ok


def register_engine(engine: "Engine | types.ModuleType") -> None:
...
def load_engines(engine_list: list[dict[str, t.Any]]) -> dict[str, Engine | ModuleType]:
    ...
    for engine_data in engine_list:
        ...
        if engine:
            ...
            logger.error(
                "loading engine %s failed: set engine to inactive!",
