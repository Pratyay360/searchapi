...
def request(query: str, params: dict[str, t.Any]) -> None:
    ...
    ui_lang = locales.get_engine_locale(
        params["searxng_locale"], traits.custom["ui_lang"], "en-us"
    )
...
def _parse_search(resp: SXNG_Response) -> EngineResults:
    ...
    for result in eval_xpath_list(dom, "//div[contains(@class, 'snippet ')]"):
        ...
        title_tag = eval_xpath_getindex(
            result, ".//div[contains(@class, 'title')]", 0, default=None
        )
        if (
            url is None or title_tag is None or not urlparse(url).netloc
        ...
        if len(_content):
            ...
            _pub_date = extract_text(
                eval_xpath_getindex(
                    _content, ".//span[contains(@class, 't-secondary')]", 0, default=""
                )
            )
            if _pub_date:
        ...
        thumbnail: str = eval_xpath_getindex(
            result, ".//a[contains(@class, 'thumbnail')]//img/@src", 0, default=""
        )
...
def _parse_news(resp: SXNG_Response) -> EngineResults:
    ...
    for result in eval_xpath_list(
        dom, "//div[contains(@class, 'results')]//div[@data-type='news']"
    ):
        url = eval_xpath_getindex(
            result, ".//a[contains(@class, 'result-header')]/@href", 0, default=None
        )
        if url is None:
            continue
        ...
        thumbnail = eval_xpath_getindex(
            result, ".//div[contains(@class, 'image-wrapper')]//img/@src", 0, default=""
        )
...
def fetch_traits(engine_traits: EngineTraits) -> None:
    ...
    for k, v in country_tags.items():
        ...
        for lang_tag in babel.languages.get_official_languages(
            country_tag, de_facto=True
        ):
            lang_tag = lang_map.get(lang_tag, lang_tag)
            try:
                sxng_tag = region_tag(
                    babel.Locale.parse(f"{lang_tag}_{country_tag.upper()}")
                )
            except babel.UnknownLocaleError:
                # silently ignore unknown languages
                continue
