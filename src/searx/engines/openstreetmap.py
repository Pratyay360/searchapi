...
from searx.engines.wikidata import (
    get_thumbnail,
    send_wikidata_query,
...
def value_wikipedia_link(value) -> tuple[str, str]:
    value = value.split(":", 1)
    return (
        "https://{}.wikipedia.org/wiki/{}".format(*value),
        "{1} ({0})".format(*value),
    )
...
def get_wikipedia_image(raw_value) -> str | None:
    if not raw_value:
        return None
    return get_external_url("wikimedia_image", raw_value)


def fetch_wikidata(nominatim_json, user_language) -> None:
    ...
    if wikidata_ids:
        ...
        query = wikidata_image_sparql.replace(
            "%WIKIDATA_IDS%", sparql_string_escape(wikidata_ids_str)
        ).replace("%LANGUAGE%", sparql_string_escape(user_language))
        ...
        for wd_result in wikidata_json.get("results", {}).get("bindings", {}):
            ...
            for result in wd_to_results.get(wd_id, []):
                result["wikidata"] = {
                    ...
                    "image": get_wikipedia_image(
                        wd_result.get("image", {}).get("value")
                    ),
                    "image_sign": get_wikipedia_image(
                        wd_result.get("sign", {}).get("value")
                    ),
                    "image_symbol": get_wikipedia_image(
...
def get_key_label(key_name: str, lang):
