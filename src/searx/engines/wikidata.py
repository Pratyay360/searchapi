...
from searx.utils import get_string_replaces_function, searxng_useragent
...
DUMMY_ENTITY_URLS = {
    "http://www.wikidata.org/entity/" + wid
...
class WDAttribute:
    ...
    def get_select(self) -> str:
        return "(group_concat(distinct ?{name};separator=', ') as ?{name}s)".replace(
            "{name}", self.name
        )
...
class WDArticle(WDAttribute):
    def __init__(self, language: str, kwargs: dict[str, t.Any] | None = None) -> None:
    ...
    def get_select(self) -> str:
        return "?article{language} ?articleName{language}".replace(
...
class WDLabelAttribute(WDAttribute):
    def get_select(self) -> str:
        return "(group_concat(distinct ?{name}Label;separator=', ') as ?{name}Labels)".replace(
            "{name}", self.name
        )
...
class WDImageAttribute(WDURLAttribute):
    def __init__(
        self, name: str, url_id: str | None = None, priority: int = 100
    ) -> None:
...
class WDDateAttribute(WDAttribute):
    def get_select(self) -> str:
        return (
            "?{name} ?{name}timePrecision ?{name}timeZone ?{name}timeCalendar".replace(
                "{name}", self.name
            )
        )
...
def send_wikidata_query(
    query: str, method: str = "GET", **kwargs: dict[str, t.Any]
) -> dict[str, t.Any]:
    if method == "GET":
        # query will be cached by wikidata
        http_response = get(
            SPARQL_ENDPOINT_URL + "?" + urlencode({"query": query}),
            headers=get_headers(),
            **kwargs,
        )
    else:
        ...
        http_response = post(
            SPARQL_ENDPOINT_URL, data={"query": query}, headers=get_headers(), **kwargs
        )
...
def response(resp: "SXNG_Response") -> list[dict[str, t.Any]]:
    ...
    for result in jsonresponse.get("results", {}).get("bindings", []):
        ...
        if entity_url not in seen_entities and entity_url not in DUMMY_ENTITY_URLS:
            ...
            logger.debug(
                "The SPARQL request returns duplicate entities: %s",
                str(attribute_result),
            )
...
def get_thumbnail(img_src: str | None) -> str | None:
    ...
    if not img_src is None and _IMG_SRC_DEFAULT_URL_PREFIX in img_src.split()[0]:
        ...
        img_src_size = img_src_size[
            img_src_size.index("=") + 1 : img_src_size.index("&")
        ]
            ...
            if isinstance(attribute, (WDURLAttribute, WDArticle)):
                ...
                for url in value.split(", "):
                    ...
                    if "list" in display_type and (
                        attribute.kwargs.get("official")
                        or isinstance(attribute, WDArticle)
                    ):
                        results.append(
                            {
                                "title": infobox_title,
                    ...
                    if isinstance(attribute, WDArticle) and (
                        (attribute.language == "en" and infobox_id_lang is None)
...
def get_query(query: str, language: str) -> tuple[str, WDAttrList]:
    ...
    wikibase_label = list(
        filter(lambda s: len(s) > 0, [a.get_wikibase_label() for a in attributes])
    )
    group_by = list(
        filter(lambda s: len(s) > 0, [a.get_group_by() for a in attributes])
    )
    query = (
...
def get_attributes(language: str):
    ...
    def add_image(name: str, url_id: str | None = None, priority: int = 1) -> None:
        attributes.append(WDImageAttribute(name, url_id, priority))

    def add_date(name: str) -> None:
...
def debug_explain_wikidata_query(query: str, method: str = "GET") -> bytes:
    if method == "GET":
        http_response = get(
            SPARQL_EXPLAIN_URL + "&" + urlencode({"query": query}),
            headers=get_headers(),
        )
    else:
        http_response = post(
            SPARQL_EXPLAIN_URL, data={"query": query}, headers=get_headers()
        )
    http_response.raise_for_status()
...
def init_wikidata_properties() -> None:
