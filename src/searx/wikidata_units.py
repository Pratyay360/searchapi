...
class Beaufort:
    ...
    def from_si(cls, value) -> float:
        if value < 0 or value > 55.6:
            raise ValueError(
                f"invalid value {value} / the Beaufort scales from 0 to 16 (55.6 m/s)"
            )
        bft = 0
...
def convert_from_si(si_name: str, symbol: str, value: float) -> float:
...
def convert_to_si(si_name: str, symbol: str, value: float) -> float:
...
def units_by_si_name(si_name: str):
