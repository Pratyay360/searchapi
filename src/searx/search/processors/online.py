...
class OnlineProcessor(EngineProcessor):
    ...
    def init_engine(self) -> bool:
        ...
        self.init_network_in_thread(
            start_time=default_timer(), timeout_limit=self.engine.timeout
        )
        return super().init_engine()

    def init_network_in_thread(self, start_time: float, timeout_limit: float) -> None:
    ...
    def get_params(
        self, search_query: "SearchQuery", engine_category: str
    ) -> OnlineParams | None:
        ...
        base_params: RequestParams | None = super().get_params(
            search_query, engine_category
        )
        ...
        self.logger.debug(
            "HTTP Accept-Language: %s", headers.get("Accept-Language", "")
        )

        return params

    def _send_http_request(self, params: OnlineParams) -> SXNG_Response:
    ...
    def search(  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        try:
            ...
            self.logger.error(
                f"SSLError {e}, verify={searx.network.get_network(self.engine.name).verify}"
            )
        except (TimeoutError, httpx.TimeoutException) as e:
            # requests timeout (connect or read)
            self.handle_exception(result_container, e, suspend=True)
            self.logger.error(
                f"HTTP requests timeout (search duration : {default_timer() - start_time} s, timeout: {timeout_limit} s) : {e.__class__.__name__}"
            )
        except (httpx.HTTPError, httpx.StreamError) as e:
            # other requests exception
            self.handle_exception(result_container, e, suspend=True)
            self.logger.exception(
                f"requests exception (search duration : {default_timer() - start_time} s, timeout: {timeout_limit} s) : {e}"
            )
        except (
            SearxEngineCaptchaException,
            SearxEngineTooManyRequestsException,
