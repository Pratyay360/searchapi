...
class SuspendedStatus:
    ...
    def suspend(self, suspended_time: int | None, suspend_reason: str) -> None:
        with self.lock:
...
class EngineProcessor(ABC):
    ...
    def __init__(self, engine: "Engine|types.ModuleType") -> None:
        ...
        self.suspended_status: SuspendedStatus = SUSPENDED_STATUS.setdefault(
            key, SuspendedStatus()
        )

    def initialize(self, callback: t.Callable[["EngineProcessor", bool], bool]) -> None:
        ...
        *registration* (the ``callback``) may occur later and is not already
        ...
        if not callable(self.engine.init):
            logger.error(
                "Engine's init method isn't a callable (is of type: %s).",
                type(self.engine.init),
            )
            callback(self, False)
            return

        def __init_processor_thread() -> None:
    ...
    def init_engine(self) -> bool:
        ...
        try:
            ...
            logger.exception(
                "Init method of engine %s failed due to an exception.", self.engine.name
            )
            init_ok = False
    ...
    def _extend_container_basic(
        ...
        if page_load_time is not None:
            histogram_observe(
                page_load_time, "engine", self.engine.name, "time", "http"
            )
