...
type OnlineParamTypes = (
    OnlineParams | OnlineDictParams | OnlineCurrenciesParams | OnlineUrlSearchParams
)
type OfflineParamTypes = RequestParams
type ParamTypes = OfflineParamTypes | OnlineParamTypes
...
class ProcessorMap(dict[str, EngineProcessor]):
    ...
    def init(self, engine_list: list[dict[str, t.Any]]) -> None:
    ...
    def register_processor(self, eng_proc: EngineProcessor, eng_proc_ok: bool) -> bool:
        ...
        if eng_proc_ok:
            ...
            logger.error(
                "can't register engine processor: %s (init failed)",
                eng_proc.engine.name,
            )
