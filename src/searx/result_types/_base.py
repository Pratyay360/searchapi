...
def _normalize_url_fields(result: "Result | LegacyResult") -> None:
...
def _normalize_text_fields(result: "MainResult | LegacyResult") -> None:
...
def _filter_urls(
    ...
    filter_func: "Callable[[Result | LegacyResult, str, str], str | bool]",
) -> None:
    ...
    for field_name in url_fields:
        ...
        if isinstance(new_url, bool):
            ...
            log.debug(
                "filter_urls: modify field %s URL %s -> %s",
                field_name,
                url_src,
                new_url,
            )
        ...
        for item in infobox_urls:
            ...
            if isinstance(new_url, bool):
                if new_url:
                    ...
                    continue
                log.debug(
                    "filter_urls: remove URL from field 'urls' ('infobox_urls') URL %s",
                    url_src,
                )
                new_url = None
            if new_url:
                log.debug(
                    "filter_urls: modify URL from field 'urls' ('infobox_urls') URL %s -> %s",
                    url_src,
                    new_url,
                )
                item["url"] = new_url
        ...
        for item in infobox_attributes:
            ...
            if isinstance(new_url, bool):
                if new_url:
                    ...
                    continue
                log.debug(
                    "filter_urls: drop field 'image.src' ('infobox_attributes') URL %s",
                    url_src,
                )
                new_url = None
...
def _normalize_date_fields(result: "MainResult | LegacyResult") -> None:
...
class Result(msgspec.Struct, kw_only=True):
    ...
    def filter_urls(
        self, filter_func: "Callable[[Result | LegacyResult, str, str], str | bool]"
    ) -> None:
    ...
    def __setitem__(self, field_name: str, value: t.Any) -> None:
    ...
    def defaults_from(self, other: "Result") -> None:
...
class LegacyResult(dict[str, t.Any]):
    ...
    def __setattr__(self, name: str, val: t.Any) -> None:
        self[name] = val
    ...
    def __hash__(self) -> int:  # pyright: ignore[reportIncompatibleVariableOverride]
        ...
        if not any(
            cls in self
            for cls in ["suggestion", "correction", "infobox", "engine_data"]
        ):
    ...
    def defaults_from(self, other: "LegacyResult") -> None:
