...
type CacheRowType = tuple[str, typing.Any, int | None]
    ...
    def __post_init__(self) -> None:
        ...
        if not self.db_url:
            self.db_url = (
                tempfile.gettempdir()
                + os.sep
                + f"sxng_cache_{ExpireCache.normalize_name(self.name)}.db"
            )
    ...
    def report(self) -> str:
        ...
        for ctx_name, kv_list in self.cached_items.items():
            ...
            for key, value, expire in kv_list:
                valid_until = ""
                if expire:
                    valid_until = datetime.datetime.fromtimestamp(expire).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                c_kv += 1
                value_str = str(value)
...
class ExpireCache(abc.ABC):
    ...
    @abc.abstractmethod
    def set(
        self, key: str, value: typing.Any, expire: int | None, ctx: str | None = None
    ) -> bool:
    ...
    @abc.abstractmethod
    def get(
        self, key: str, default: typing.Any = None, ctx: str | None = None
    ) -> typing.Any:
...
class ExpireCacheSQLite(sqlitedb.SQLiteAppl, ExpireCache):
    ...
    def __init__(self, cfg: ExpireCacheCfg) -> None:
    ...
    def init(self, conn: sqlite3.Connection) -> bool:
        ...
        if old != new:
            if old is not None:
                log.warning(
                    "[%s] hash token changed: truncate all cache tables", self.cfg.name
                )
    ...
    def maintenance(self, force: bool = False, truncate: bool = False) -> bool:
        ...
        self.properties.set(
            "LAST_MAINTENANCE", ""
        )  # hint: this (also) sets the m_time of the property!
        ...
        with self.connect() as conn:
            for table in self.table_names:
                ...
                log.debug(
                    "deleted %s keys from table %s (expire date reached)",
                    res.rowcount,
                    table,
                )
    ...
    def next_maintenance_time(self) -> int:
        ...
        return self.cfg.MAINTENANCE_PERIOD + self.properties.m_time(
    ...
    def set(
        ...
        if c:
            log.debug(
                "%s -- %s: key '%s' updated or inserted (%s errors)",
                self.cfg.name,
                ctx,
                key,
                len(err_msg_list),
    ...
    def _setmany(
        ...
        for key, _val, expire in opt_list:
            ...
            if len(value) > self.cfg.MAX_VALUE_LEN:
                err_msg_list.append(
                    f"{table}.key='{key}' - serialized value too big to cache (len: {len(value)}) "
                )
                continue
