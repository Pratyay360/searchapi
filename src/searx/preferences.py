...
class Setting:
    ...
    def __init__(self, default_value: t.Any, locked: bool = False) -> None:
        super().__init__()
...
class EnumStringSetting(Setting):
    ...
    def __init__(
        self, default_value: str, choices: Iterable[str], locked: bool = False
    ) -> None:
        super().__init__(default_value, locked)
        self.choices: Iterable[str] = choices
        self._validate_selection(self.value)

    def _validate_selection(self, selection: str) -> None:
...
class MultipleChoiceSetting(Setting):
    ...
    def __init__(
        self, default_value: list[str], choices: Iterable[str], locked: bool = False
    ) -> None:
        super().__init__(default_value, locked)
        self.choices: Iterable[str] = choices
        self._validate_selections(self.value)

    def _validate_selections(self, selections: list[str]) -> None:
    ...
    def parse_form(self, data: list[str]) -> None:
        if self.locked:
            return
...
class SetSetting(Setting):
    ...
    def parse_form(self, data: str) -> None:
        if self.locked:
            return
...
class SearchLanguageSetting(EnumStringSetting):
    ...
    def _validate_selection(self, selection: str) -> None:
        if (
            selection != ""
            and selection != "auto"
            and not VALID_LANGUAGE_CODE.match(selection)
        ):
...
class BooleanChoices:
    ...
    def transform_form_items(self, items: list[str]) -> list[str]:
        return items

    def transform_values(self, values: list[str]) -> list[str]:
        return values

    def parse_cookie(self, data_disabled: str, data_enabled: str) -> None:
    ...
    def parse_form(self, items: list[str]) -> None:
        if self.locked:
            return
    ...
    @property
    def enabled(self) -> Generator[str]:
...
class EnginesSetting(BooleanChoices):
    ...
    def __init__(self, default_value, engines: Iterable[Engine]) -> None:
        choices = {}
    ...
    def transform_form_items(self, items):
        return [
            item[len("engine_") :].replace("_", " ").replace("  ", "__")
            for item in items
        ]
...
class PluginsSetting(BooleanChoices):
    ...
    def __init__(self, default_value, plugins: Iterable[searx.plugins.Plugin]) -> None:
        super().__init__(
            default_value, {plugin.id: plugin.active for plugin in plugins}
        )
...
class ClientPref:
    ...
    def __init__(self, locale: babel.Locale | None = None) -> None:
        self.locale = locale
    ...
    @classmethod
    def from_http_request(cls, http_request: SXNG_Request) -> Self:
...
class Preferences:
    ...
    def __init__(
        ...
        self.key_value_settings: dict[str, Setting] = {
            ...
            "image_proxy": BooleanSetting(
                settings["server"]["image_proxy"], locked=is_locked("image_proxy")
            ),
            ...
            "center_alignment": BooleanSetting(
                settings["ui"]["center_alignment"], locked=is_locked("center_alignment")
            ),
            "advanced_search": BooleanSetting(
                settings["ui"]["advanced_search"], locked=is_locked("advanced_search")
            ),
            "query_in_title": BooleanSetting(
                settings["ui"]["query_in_title"], locked=is_locked("query_in_title")
            ),
            ...
            "hotkeys": EnumStringSetting(
                settings["ui"]["hotkeys"], choices=["default", "vim"]
            ),
            "url_formatting": EnumStringSetting(
                settings["ui"]["url_formatting"], choices=["pretty", "full", "host"]
            ),
            # fmt: on
        }
    ...
    def get_as_url_params(self) -> str:
    ...
    def parse_encoded_data(self, input_data: str) -> None:
        """parse (base64) preferences from request (``.request.form['preferences']``)"""
    ...
    def parse_dict(self, input_data: dict[str, str]) -> None:
        """parse preferences from request (``.request.form``)"""
    ...
    def parse_form(self, input_data: dict[str, str]) -> None:
        """Parse formular (``<input>``) data from a ``.request.form``"""
        disabled_engines = []
        enabled_categories = []
        disabled_plugins = []
