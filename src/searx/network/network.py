...
from .client import AsyncHTTPTransportNoHttp, get_loop, new_client
...
class Network:
    ...
    def __init__(
        ...
        logger_name: str | None = None,  # pyright: ignore[reportArgumentType]
    ) -> None:
    ...
    def get_proxy_cycles(
        self,
    ) -> Generator[tuple[tuple[str, str], ...], str, str]:  # not sure type is correct
    ...
    async def check_tor_proxy(
        client: httpx.AsyncClient, proxies: tuple[tuple[str, str], ...]
    ) -> bool:
        ...
        for transport in client._mounts.values():  # pylint: disable=protected-access
            if isinstance(transport, AsyncHTTPTransportNoHttp):
                continue
            if transport._pool and getattr(
    ...
    async def get_client(
        self, verify: bool | None = None, max_redirects: int | None = None
    ) -> httpx.AsyncClient:
    ...
    def patch_response(
        self, response: httpx.Response, do_raise_for_httperror: bool
    ) -> SXNG_Response:
...
def check_network_configuration() -> None:
    async def check():
        exception_count = 0
...
def initialize(
    ...
    def new_network(
        params: dict[str, t.Any], logger_name: str | None = None
    ) -> Network:
    ...
    if "image_proxy" not in NETWORKS:
        ...
        NETWORKS["image_proxy"] = new_network(
            image_proxy_params, logger_name="image_proxy"
        )
