...
def reset_time_for_thread() -> None:
...
def set_timeout_for_thread(timeout: float, start_time: float | None = None) -> None:
    THREADLOCAL.timeout = timeout
    THREADLOCAL.start_time = start_time


def set_context_network_name(network_name: str) -> None:
...
def multi_requests(request_list: list["Request"]) -> list[httpx.Response | Exception]:
    ...
    with _record_http_time() as start_time:
        ...
        for request_desc in request_list:
            ...
            future = asyncio.run_coroutine_threadsafe(
                network.request(
                    request_desc.method, request_desc.url, **request_desc.kwargs
                ),
                loop,
            )
...
class Request:
    ...
    @staticmethod
    def get(url: str, **kwargs: t.Any) -> Request:
        return Request("GET", url, kwargs)

    @staticmethod
    def options(url: str, **kwargs: t.Any) -> Request:
        return Request("OPTIONS", url, kwargs)

    @staticmethod
    def head(url: str, **kwargs: t.Any) -> Request:
        return Request("HEAD", url, kwargs)

    @staticmethod
    def post(url: str, **kwargs: t.Any) -> Request:
        return Request("POST", url, kwargs)

    @staticmethod
    def put(url: str, **kwargs: t.Any) -> Request:
        return Request("PUT", url, kwargs)

    @staticmethod
    def patch(url: str, **kwargs: t.Any) -> Request:
        return Request("PATCH", url, kwargs)

    @staticmethod
    def delete(url: str, **kwargs: t.Any) -> Request:
...
def post(
    url: str, data: dict[str, t.Any] | None = None, **kwargs: t.Any
) -> SXNG_Response:
...
async def stream_chunk_to_queue(
    network: Network, queue, method: str, url: str, **kwargs: t.Any
):
    try:
...
def _close_response_method(self) -> None:
...
def stream(
    method: str, url: str, **kwargs: t.Any
) -> tuple[SXNG_Response, Iterable[bytes]]:
