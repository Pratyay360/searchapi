...
class Histogram:  # pylint: disable=missing-class-docstring
    ...
    def __init__(self, width: int = 10, size: int = 200) -> None:
    ...
    @property
    def quartile_percentage(self) -> list[int]:
    ...
    @property
    def quartile_percentage_map(self) -> dict[float, int]:
        result = {}
    ...
    def percentage(self, percentage) -> Decimal | None:
        # use Decimal to avoid rounding errors
        x = decimal.Decimal(0)
        width = decimal.Decimal(self._width)
    ...
    def __repr__(self) -> str:
        return (
            "Histogram<avg: " + str(self.average) + ", count: " + str(self._count) + ">"
        )
...
class HistogramStorage:  # pylint: disable=missing-class-docstring
    ...
    def __init__(self, histogram_class: type[Histogram] = Histogram) -> None:
        self.clear()
    ...
    def configure(self, width, size, *args) -> Histogram:
...
class CounterStorage:  # pylint: disable=missing-class-docstring
    ...
    def add(self, value: int, *args: str) -> None:
        with self.lock:
