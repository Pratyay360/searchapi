...
class ErrorContext:  # pylint: disable=missing-class-docstring
    ...
    def __repr__(self) -> str:
        return f"ErrorContext({self.filename!r}, {self.line_no!r}, {self.code!r}, {self.exception_classname!r}, {self.log_message!r}, {self.log_parameters!r}) {self.secondary!r}"
...
def get_trace(traces: list[FrameInfo]) -> FrameInfo:
...
def get_messages(exc: BaseException, filename: str) -> tuple[str, ...]:  # pylint: disable=too-many-return-statements
...
def get_error_context(
    framerecords: list[FrameInfo],
    exception_classname: str | None,
...
def count_exception(
    ...
    try:
        ...
        error_context = get_error_context(
            framerecords, exception_classname, None, log_parameters, secondary
        )
...
def count_error(
    ...
    try:
        error_context = get_error_context(
            framerecords, None, log_message, log_parameters or (), secondary
        )
        add_error_context(engine_name, error_context)
    finally:
        del framerecords
