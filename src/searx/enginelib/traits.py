...
class EngineTraits:
    ...
    def get_language(
        self, searxng_locale: str, default: str | None = None
    ) -> str | None:
        ...
        return locales.get_engine_locale(
    ...
    def fetch_traits(cls, log: t.Callable[[str], None]) -> "EngineTraitsMap":
        ...
        for engine_name in names:
            ...
            try:
                ...
                if v:
                    log(
                        f"WARNING: re-use old values from fetch_traits - ENGINE_TRAITS[{engine_name}]"
                    )
                    traits = EngineTraits(**v)
                else:
                    log(
                        f"WARNING: no old values available for ENGINE_TRAITS[{engine_name}], skipping"
                    )

            if traits is not None:
                log(
                    "%-20s: SearXNG languages --> %s "
                    % (engine_name, len(traits.languages))
                )
                log(
                    "%-20s: SearXNG regions   --> %s"
                    % (engine_name, len(traits.regions))
                )
