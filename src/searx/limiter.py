...
def get_cfg() -> config.Config:
    ...
    if CFG is None:
        ...
        cfg_file = (
            settings_loader.get_user_cfg_folder() or Path("/etc/searxng")
        ) / "limiter.toml"
...
def filter_request(request: SXNG_Request) -> werkzeug.Response | None:
    ...
    for func in [
        ...
        if val is not None:
            logger.debug(
                f"NOT OK ({func.__name__}): {network}: %s", dump_request(sxng_request)
            )
            return val
    ...
    if request.path == "/search":
        for func in [
            ...
            if val is not None:
                logger.debug(
                    f"NOT OK ({func.__name__}): {network}: %s",
                    dump_request(sxng_request),
                )
                return val
